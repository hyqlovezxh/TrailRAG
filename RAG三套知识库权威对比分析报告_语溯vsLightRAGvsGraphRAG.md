...
**开源归属**：本项目（TrailRAG / 语溯RAG）参考了以下仓库的源码，并依照各自开源协议署名——[Yuxi](https://github.com/xerrors/Yuxi)（知识库 / 知识图谱构建与检索能力）、[LightRAG](https://github.com/HKUDS/LightRAG)（图谱 WebUI 实现方式）。
...
| 系统 | 仓库路径 | 源码完整度 | 定位 |
|---|---|---|---|
| 语溯RAG | `D:/PK/语溯RAG` | 完整（64 .py） | 面向低质量多源数据的全模态 RAG |
...
### 1.2 本次对比要解决的核心问题
传统知识库（含 GraphRAG、LightRAG 等）大多面向**企业高质量知识**设计：文档规范、实体清晰、少矛盾，因此对**确定性、高质量语料**效果好。但真实世界的数据往往相反：**不确定性高、海量、低质量**。一个典型例子是调查/取证材料——聊天记录杂乱、口语化、含大量无关噪声，笔录、聊天、资金流水、话单、卡口之间**可能相互矛盾**（缺乏数据治理），数据海量且高度不确定：

- 聊天记录**杂乱、口语化、含大量无关噪声**；
- 笔录、聊天、资金流水、话单、卡口之间**可能相互矛盾**（缺乏数据治理）；
- 数据**海量、低质量、且高度不确定**。

在缺乏数据治理的低质量语料上，传统知识库很难从噪声中挖掘出用户真正需要的真实线索，构建出的知识图谱往往夹杂大量无关与低质量节点。**因此本报告的检索评测不只看"语义检索准不准"，而是重点评估知识库在"不确定性数据 + 海量低质量数据 + 跨证据多跳 + 宏观时序重建"下的真实可用性**。该评测场景统一折算进「检索准确度」维度（详见 §3.1、§4.1 与 §6），不单列维度，以同时满足"可量化对比"与"不喧宾夺主"。这里的"取证/证据挖掘专项"仅为这类低质量、多矛盾点数据的一个代表性例子，用于检验知识库在低质量数据下的可用性。
...
- **利益披露**：语溯RAG 为面向本场景（低质量/不确定性数据）深度定制的系统，其在"低质量适配"相关维度占优是**目标场景对齐的结果**，非评分偏向；本报告同时公布等权口径，并对语溯RAG 的短板（健壮性、Token 效率、错误容忍度）如实给出低于对手的分数，以证明评分未被人为拔高。
...
### 4.8 可解释性【5 / 4 / 4】
- **语溯RAG 5**：来源引用 + 证据链 + 图谱路径 + **可解释闭环**（词法精确命中 `matched_exact_tokens` 可回溯，`milvus.py:1720-1748`），契合低质量数据下的可核验需求。
...
### 6.4 实测对比（TrailRAG 已实测 / LightRAG、GraphRAG 待替换）

> 本节为同一套基准测试题上的**真实端到端评测**。为使对比最具代表性，测试集刻意采用**混合数据**：聊天记录、笔录、资金流水、取证数据等多维材料混排，**数据矛盾点多、噪声大，普通 RAG 易误判**，最能体现 RAG 面对复杂数据的真实检索能力（即 §1.2 所描述的低质量、多矛盾点数据场景）。

**TrailRAG（语溯RAG）—— 已实测（真实成绩）**
...
**LightRAG / GraphRAG —— 答案准确率已知，明细待实测替换**

> 用户实测：LightRAG 与 GraphRAG 的**答案准确率分别为 88% 与 92%**；但完整的召回率等明细数据暂已丢失。**以下占位采用「待实测替换（PENDING REAL MEASUREMENT）」，不虚构召回率数值**。待用户完成实测后，将用真实数据替换占位，避免误导开源社区。

| 项目 | LightRAG | GraphRAG |
|---|---|---|
| 答案准确率 | **88%** | **92%** |
| 召回率 @1 | 待实测替换 | 待实测替换 |
| 召回率 @3 | 待实测替换 | 待实测替换 |
...
**口径说明**：以上三套系统在**同一套基准测试题**（混合低质量、多矛盾点数据）下评测；TrailRAG 为端到端实跑结果，LightRAG / GraphRAG 待用户补足明细后替换占位。
