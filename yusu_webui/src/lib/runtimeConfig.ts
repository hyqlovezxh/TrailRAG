 ...
 * REQUEST time: the FastAPI server replaces a `<!-- __YUSU_RUNTIME_CONFIG__ -->`
 * comment in `index.html` with a `<script>window.__YUSU_CONFIG__ = ...</script>`
 * snippet built from `YUSU_API_PREFIX` / `YUSU_WEBUI_PATH`. This module
...
const config =
  (typeof window !== 'undefined' && window.__YUSU_CONFIG__) || {}
