...
Ported from YUSU ``yuxi.models.providers.service``; the YUSU ``repository``
dependency (trimmed from the reference tree) is replaced by the SQLite
``ModelProviderRepository``, and ``resolve_api_key`` reads environment
variables only (the DB never stores real keys).
"""
...
from yusu_kb.repositories.model_provider_repository import ModelProviderRepository
from yusu_kb.storage.sqlite.models_knowledge import ModelProvider


class _RemoteModelError(ValueError):
    """Raised when a provider's model-listing endpoint returns an unusable response."""
...
def _normalize_model_list(models: Any) -> list[dict[str, Any]]:
    ...
    for item in _normalize_list(models):
        if not isinstance(item, dict):
            raise ValueError("模型配置必须是对象列表")  # noqa: TRY004 - 与 YUSU 业务语义一致
...
def _normalize_payload(data: dict[str, Any], *, partial: bool = False) -> dict[str, Any]:
    ...
    elif provider_type is not None and provider_type not in VALID_PROVIDER_TYPES:
...
def resolve_api_key(provider: ModelProvider) -> str | None:
    """解析 provider 的 API Key：只从 api_key_env 环境变量读取（DB 永不存真实 key）。"""
...
def _normalize_remote_model(raw_model: dict[str, Any], model_type: str = "chat") -> dict[str, Any]:
    # Ollama and some gateways expose the model id under ``name`` instead of ``id``.
...
async def get_all_model_providers() -> list[ModelProvider]:
    """获取全部独立模型供应商配置。"""
    return await ModelProviderRepository().get_all()


async def get_model_provider_by_id(provider_id: str) -> ModelProvider | None:
    """按 provider_id 获取独立模型供应商配置。"""
    return await ModelProviderRepository().get_by_provider_id(provider_id)


async def ensure_builtin_model_providers_in_db() -> None:
    ...
    for provider_def in BUILTIN_PROVIDERS:
        ...
        if existing_provider:
            if not existing_provider.enabled_models and provider_def.get("enabled_models"):
                await repository.update(
                    provider_id,
                    {
                        "enabled_models": _normalize_model_list(provider_def["enabled_models"]),
                        "capabilities": provider_def.get("capabilities") or existing_provider.capabilities,
                        "updated_by": "system",
                    },
                )
            continue
        ...
        await repository.create(_normalize_payload(payload))


async def create_provider_config(data: dict[str, Any], username: str) -> ModelProvider:
    """创建独立模型供应商配置。"""
    repository = ModelProviderRepository()
    payload = _normalize_payload(data)
    if await repository.get_by_provider_id(payload["provider_id"]):
...
async def update_provider_config(
    ...
    """更新独立模型供应商配置。"""
    repository = ModelProviderRepository()
    provider = await repository.get_by_provider_id(provider_id)
    if provider is None:
        return None
    ...
    payload["updated_by"] = username
    return await repository.update(provider_id, payload)


async def delete_provider_config(provider_id: str) -> bool:
    """删除独立模型供应商配置。"""
    return await ModelProviderRepository().delete(provider_id)
...
async def _fetch_models_from_endpoint(
    ...
    try:
        ...
        raise _RemoteModelError(
            f"{endpoint} 返回的不是合法 JSON（可能是代理/网关错误页）"
        ) from exc

    # Tolerant of multiple provider response shapes:
    # OpenAI-compatible -> {"data": [...]}; Ollama/Gemini -> {"models": [...]};
    # some gateways return a bare top-level list.
    ...
    if not isinstance(raw_models, list):
        raise _RemoteModelError(f"{endpoint} 响应必须是列表或包含 data/models 列表")
...
async def fetch_remote_models(provider: ModelProvider, *, transport=None) -> list[dict[str, Any]]:
    ...
    ``transport`` 是测试注入口（httpx.MockTransport）。
    """
    ...
    # Build the endpoint list from the provider's configured endpoints rather
    # than from ``capabilities`` so that embedding/rerank endpoints are queried
    # even when the provider template omits an explicit capability declaration.
    endpoint_specs: list[tuple[str | None, str]] = [
        (provider.models_endpoint, "chat"),
    ]
    if provider.embedding_models_endpoint:
        endpoint_specs.append((provider.embedding_models_endpoint, "embedding"))
    if provider.rerank_models_endpoint:
        endpoint_specs.append((provider.rerank_models_endpoint, "rerank"))

    seen_ids: set[tuple[str, str]] = set()
    models: list[dict[str, Any]] = []
    first_error: BaseException | None = None
    client_kwargs: dict[str, Any] = {"timeout": 40.0}
    ...
    async with httpx.AsyncClient(**client_kwargs) as client:
        ...
        for result in results:
            if isinstance(result, Exception):
                # Keep the first failure for diagnostics but don't discard the
                # models successfully fetched from other endpoints.
...
def _build_model_from_info(info, *, transport=None) -> Any:
    """按 ModelInfo 构造当前项目的运行时模型实例（chat/embedding/rerank）。
    ...
    if info.model_type == "rerank":
        from yusu_kb.models.rerank import OpenAIReranker
...
async def test_model_status_by_spec(spec: str, *, transport=None) -> dict:
    """根据 spec 测试模型连接状态。"""
    ...
    try:
        model = _build_model_from_info(info, transport=transport)
        if info.model_type in ("embedding", "rerank"):
