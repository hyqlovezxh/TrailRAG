"""RAG 评估服务（适配自 YUSU ``yuxi.knowledge.eval.service``，832 行）。

关键适配：
- 无任务系统（tasker/TaskRepository）→ 生成数据集/评估运行改为进程内
  ``asyncio.create_task``，由 ``EvaluationService`` 跟踪（``_tasks``），
  支持取消与进程重启后的状态收敛（running 且无存活任务超 300s → failed）。
- ``select_model`` 全局解析 → 注入 ``select_model_fn(model_spec)``（默认走
  ``create_chat_model(default_spec=...)``），测试可注入 FakeChat。
- ``knowledge_base.aget_kb`` → 注入 ``kb_manager``（默认取 ``api.deps``
  的全局 manager），仅支持 ``local`` 类型知识库。
"""

import asyncio
import json
import re
import uuid
from typing import Any

from yusu_kb.knowledge.base import KBNotFoundError
from yusu_kb.knowledge.eval.benchmark_generation import (
    dump_benchmark_item,
    iter_generated_benchmark_items,
    normalize_generation_concurrency_count,
)
from yusu_kb.knowledge.eval.evaluator import aggregate_metrics, evaluate_question
from yusu_kb.repositories.evaluation_repository import EvaluationRepository
from yusu_kb.repositories.knowledge_base_repository import KnowledgeBaseRepository
from yusu_kb.repositories.knowledge_chunk_repository import KnowledgeChunkRepository
from yusu_kb.utils.datetime_utils import format_utc_datetime, utc_now_naive
from yusu_kb.utils.logger import logger


def build_evaluation_run_name(started_at=None, hash_value: str | None = None) -> str:
    date_part = (started_at or utc_now_naive()).strftime("%Y%m%d")
    hash_part = re.sub(r"[^a-fA-F0-9]", "", hash_value or uuid.uuid4().hex).lower()[:6]
    if len(hash_part) < 6:
        hash_part = (hash_part + uuid.uuid4().hex)[:6]
    return f"eval-{date_part}-{hash_part}"


# EVAL-1: 评估并发 worker 数。受 retrieval/judge LLM 限速约束，过高会触发 rate limit。
# 评估每题需调用 answer LLM + judge LLM，实际并发请求数为 worker_count × 2，
# 因此保守取 4（8 并发请求），与 benchmark 生成默认 10 并发相当。
EVAL_WORKER_COUNT = 4

# I-18: running 但无存活后台任务的 run 收敛为 failed 的年龄下限（秒）。
_RUN_ZOMBIE_AGE_SECONDS = 300


def _default_select_model(model_spec: str) -> Any:
    """按 ``provider_id:model_id`` spec 构建 chat 适配器（默认注入实现）。"""
    from yusu_kb.models.chat import create_chat_model

    return create_chat_model(default_spec=model_spec)


class EvaluationService:
    """RAG评估服务"""

    def __init__(self, *, select_model_fn=None, kb_manager=None):
        self.eval_repo = EvaluationRepository()
        self.kb_repo = KnowledgeBaseRepository()
        self.chunk_repo = KnowledgeChunkRepository()
        # 注入点：模型工厂（默认走 ModelCache 两级解析）与 KB 管理器（默认取全局 manager）。
        self.select_model_fn = select_model_fn or _default_select_model
        self.kb_manager = kb_manager
        # 进程内后台任务：dataset_id/run_id -> asyncio.Task
        self._tasks: dict[str, asyncio.Task] = {}
        # run 进度快照（进度/消息），供 list_runs/get_run_results 实时展示
        self._run_states: dict[str, dict[str, Any]] = {}

    # ------------------------------------------------------------------
    # KB 实例解析
    # ------------------------------------------------------------------

    async def _aget_kb(self, kb_id: str) -> Any | None:
        manager = self.kb_manager
        if manager is None:
            from yusu_kb.api.deps import get_manager

            manager = await get_manager()
        try:
            return await manager._require_kb_for_database(kb_id)
        except KBNotFoundError:
            return None

    # ------------------------------------------------------------------
    # 后台任务生命周期
    # ------------------------------------------------------------------

    def _is_task_alive(self, key: str) -> bool:
        task = self._tasks.get(key)
        return task is not None and not task.done()

    def _cancel_task(self, key: str) -> bool:
        task = self._tasks.pop(key, None)
        if task is None or task.done():
            return False
        task.cancel()
        return True

    def _spawn_task(self, key: str, coro) -> None:
        """跟踪并启动后台任务（不 await，调用方轮询 DB 状态）。"""
        existing = self._tasks.get(key)
        if existing is not None and not existing.done():
            raise ValueError("任务已存在且仍在运行")

        async def runner() -> None:
            try:
                await coro
            except asyncio.CancelledError:
                raise
            except Exception as exc:  # noqa: BLE001 - 任务异常由内部处理，这里兜底记录
                logger.error(f"后台任务 {key} 异常退出: {exc}")

        self._tasks[key] = asyncio.create_task(runner())

    # ------------------------------------------------------------------
    # Dataset 序列化
    # ------------------------------------------------------------------

    def _dataset_to_dict(self, row) -> dict[str, Any]:
        return {
            "id": row.dataset_id,
            "dataset_id": row.dataset_id,
            "name": row.name,
            "description": row.description,
            "kb_id": row.kb_id,
            "item_count": row.item_count,
            "has_gold_chunks": row.has_gold_chunks,
            "has_gold_answers": row.has_gold_answers,
            "build_metadata": row.build_metadata or {},
            "created_by": row.created_by,
            "created_at": format_utc_datetime(row.created_at),
            "updated_at": format_utc_datetime(row.updated_at),
        }

    def _dataset_item_to_dict(self, item) -> dict[str, Any]:
        return {
            "item_id": item.item_id,
            "item_index": item.item_index,
            "query": item.query_text,
            "gold_chunk_ids": item.gold_chunk_ids or [],
            "gold_answer": item.gold_answer,
        }

    def _run_item_to_dict(self, item) -> dict[str, Any]:
        return {
            "query": item.query_text,
            "gold_chunk_ids": item.gold_chunk_ids,
            "gold_answer": item.gold_answer,
            "generated_answer": item.generated_answer,
            "retrieved_chunks": item.retrieved_chunks,
            "metrics": item.metrics or {},
        }

    def _is_error_run_item(self, item) -> bool:
        metrics = item.metrics or {}
        return self._metrics_is_error(metrics)

    @staticmethod
    def _metrics_is_error(metrics: dict) -> bool:
        """EVAL-2: 提取错误判定核心逻辑，支持在 upsert 前基于 metrics dict 判定。"""
        return metrics.get("score", 1.0) <= 0.5 or any(
            metrics.get(key, 1.0) < 0.3 for key in metrics if key.startswith("recall@")
        )

    def _normalize_run_name(self, name: str | None, run_id: str) -> str:
        run_name = (name or "").strip()
        if run_name:
            return run_name
        return build_evaluation_run_name(hash_value=run_id.removeprefix("run_"))

    def _run_name_from_row(self, row) -> str:
        name = (getattr(row, "name", None) or "").strip()
        if name:
            return name
        return build_evaluation_run_name(row.started_at, hash_value=row.run_id.removeprefix("run_"))

    async def _sync_dataset_build_metadata(self, row) -> None:
        """收敛生成任务状态：pending/running 但无存活进程内任务 → failed（进程重启场景）。"""
        metadata = dict(row.build_metadata or {})
        if metadata.get("source") != "generated" or metadata.get("status") not in {"pending", "running"}:
            return
        if self._is_task_alive(row.dataset_id):
            return

        metadata.pop("progress", None)
        metadata.update(status="failed", message="生成任务不存在或已中断")
        if metadata != (row.build_metadata or {}):
            await self.eval_repo.update_dataset(row.dataset_id, {"build_metadata": metadata})
            row.build_metadata = metadata

    def _build_dataset_items(
        self, dataset_id: str, kb_id: str, questions: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        return [
            {
                "item_id": f"dataset_item_{uuid.uuid4().hex[:12]}",
                "dataset_id": dataset_id,
                "kb_id": kb_id,
                "item_index": index,
                "query_text": item["query"],
                "gold_chunk_ids": item.get("gold_chunk_ids") or [],
                "gold_answer": item.get("gold_answer"),
            }
            for index, item in enumerate(questions)
        ]

    def _build_jsonl_content(self, items: list[Any]) -> str:
        lines = []
        for item in items:
            payload = {"query": item.query_text}
            if item.gold_chunk_ids:
                payload["gold_chunk_ids"] = item.gold_chunk_ids
            if item.gold_answer:
                payload["gold_answer"] = item.gold_answer
            lines.append(dump_benchmark_item(payload).rstrip("\n"))
        return "\n".join(lines) + ("\n" if lines else "")

    def _safe_jsonl_filename(self, name: str | None, fallback: str) -> str:
        filename = (name or "").strip() or fallback
        filename = re.sub(r"[\\/:*?\"<>|]+", "_", filename).strip()
        if not filename or filename in {".", ".."}:
            filename = fallback
        return filename if filename.endswith(".jsonl") else f"{filename}.jsonl"

    def _parse_jsonl_questions(self, file_content: bytes) -> tuple[list[dict[str, Any]], bool, bool]:
        questions = []
        has_gold_chunks = False
        has_gold_answers = False
        content = file_content.decode("utf-8")

        for line_num, line in enumerate(content.strip().split("\n"), 1):
            if not line.strip():
                continue
            try:
                item = json.loads(line)
            except json.JSONDecodeError as e:
                raise ValueError(f"第{line_num}行JSON格式错误: {e!s}")
            if "query" not in item:
                raise ValueError(f"第{line_num}行缺少必需的'query'字段")
            if item.get("gold_chunk_ids"):
                has_gold_chunks = True
            if item.get("gold_answer"):
                has_gold_answers = True
            questions.append(item)

        if not questions:
            raise ValueError("文件中没有有效的问题数据")
        return questions, has_gold_chunks, has_gold_answers

    # ------------------------------------------------------------------
    # Dataset CRUD
    # ------------------------------------------------------------------

    async def upload_dataset(
        self, kb_id: str, file_content: bytes, filename: str, name: str, description: str, created_by: str
    ) -> dict[str, Any]:
        try:
            questions, has_gold_chunks, has_gold_answers = self._parse_jsonl_questions(file_content)
            dataset_id = f"dataset_{uuid.uuid4().hex[:8]}"
            dataset_name = name.strip() or filename or dataset_id

            row = await self.eval_repo.create_dataset_with_items(
                {
                    "dataset_id": dataset_id,
                    "kb_id": kb_id,
                    "name": dataset_name,
                    "description": description,
                    "item_count": len(questions),
                    "has_gold_chunks": has_gold_chunks,
                    "has_gold_answers": has_gold_answers,
                    "build_metadata": {
                        "source": "upload",
                        "status": "completed",
                        "progress": 100,
                        "filename": filename,
                    },
                    "created_by": created_by,
                },
                self._build_dataset_items(dataset_id, kb_id, questions),
            )
            return self._dataset_to_dict(row)
        except Exception as e:
            logger.error(f"上传评估数据集失败: {e}")
            raise

    async def list_datasets(self, kb_id: str) -> list[dict[str, Any]]:
        try:
            rows = await self.eval_repo.list_datasets(kb_id)
            for row in rows:
                await self._sync_dataset_build_metadata(row)
            return [self._dataset_to_dict(row) for row in rows]
        except Exception as e:
            logger.error(f"获取评估数据集列表失败: {e}")
            raise

    async def get_dataset_detail(
        self, kb_id: str, dataset_id: str, page: int = 1, page_size: int = 10
    ) -> dict[str, Any]:
        try:
            row = await self.eval_repo.get_dataset(dataset_id)
            if row is None or row.kb_id != kb_id:
                raise ValueError("Dataset not found")
            # 先收敛生成任务状态（进程重启后 running → failed），保证轮询可见终态
            await self._sync_dataset_build_metadata(row)
            data = self._dataset_to_dict(row)
            if (row.build_metadata or {}).get("status", "completed") != "completed":
                # 生成任务进行中/失败：返回 200 + 基础信息与当前状态（items 为空），
                # 供前端轮询进度而非 400 中断
                data.update(
                    {
                        "items": [],
                        "pagination": {
                            "current_page": page,
                            "page_size": page_size,
                            "total_items": 0,
                            "total_pages": 0,
                            "has_next": False,
                            "has_prev": False,
                        },
                    }
                )
                return data

            total_items = await self.eval_repo.count_dataset_items(dataset_id)
            items = await self.eval_repo.list_dataset_items(dataset_id, (page - 1) * page_size, page_size)
            total_pages = (total_items + page_size - 1) // page_size
            data.update(
                {
                    "items": [self._dataset_item_to_dict(item) for item in items],
                    "pagination": {
                        "current_page": page,
                        "page_size": page_size,
                        "total_items": total_items,
                        "total_pages": total_pages,
                        "has_next": page < total_pages,
                        "has_prev": page > 1,
                    },
                }
            )
            return data
        except Exception as e:
            logger.error(f"获取评估数据集详情失败: {e}")
            raise

    async def export_dataset_jsonl(self, dataset_id: str) -> dict[str, str]:
        row = await self.eval_repo.get_dataset(dataset_id)
        if row is None:
            raise ValueError("Dataset not found")
        if (row.build_metadata or {}).get("status", "completed") != "completed":
            raise ValueError("Dataset is not ready")
        items = await self.eval_repo.list_all_dataset_items(dataset_id)
        return {
            "filename": self._safe_jsonl_filename(row.name, row.dataset_id),
            "content": self._build_jsonl_content(items),
        }

    async def delete_dataset(self, dataset_id: str) -> None:
        try:
            row = await self.eval_repo.get_dataset(dataset_id)
            if row is None:
                raise ValueError("Dataset not found")
            # 取消运行中的生成任务（进程内任务，build_metadata 中记录了 task_id）
            metadata = dict(row.build_metadata or {})
            task_id = metadata.get("task_id")
            if (
                task_id
                and metadata.get("status") in {"pending", "running"}
                and self._cancel_task(dataset_id)
            ):
                logger.info(f"已取消数据集 {dataset_id} 的生成任务 {task_id}")
            await self.eval_repo.delete_dataset(dataset_id)
            logger.info(f"成功删除评估数据集: {dataset_id}")
        except Exception as e:
            logger.error(f"删除评估数据集失败: {e}")
            raise

    # ------------------------------------------------------------------
    # Dataset 生成
    # ------------------------------------------------------------------

    async def generate_dataset(
        self,
        kb_id: str,
        name: str,
        description: str,
        count: int,
        neighbors_count: int,
        concurrency_count: int,
        llm_model_spec: str,
        generation_mode: str = "vector",
        graph_expand_top_k: int = 1,
        created_by: str = "system",
    ) -> dict[str, Any]:
        dataset_id = f"dataset_{uuid.uuid4().hex[:8]}"
        count = int(count)
        neighbors_count = int(neighbors_count)
        concurrency_count = normalize_generation_concurrency_count(concurrency_count)
        graph_expand_top_k = min(max(1, int(graph_expand_top_k)), 3)
        if generation_mode not in {"vector", "graph_enhanced"}:
            raise ValueError("不支持的评估基准生成方式")
        if generation_mode == "graph_enhanced":
            indexed_count = await self.chunk_repo.count_graph_indexed_by_kb_id(kb_id)
            if indexed_count <= 0:
                raise ValueError("当前知识库尚未完成图索引，无法使用图增强构建")
        build_metadata = {
            "source": "generated",
            "status": "pending",
            "progress": 0,
            "params": {
                "count": count,
                "neighbors_count": neighbors_count,
                "concurrency_count": concurrency_count,
                "llm_model_spec": llm_model_spec,
                "generation_mode": generation_mode,
                "graph_expand_top_k": graph_expand_top_k,
            },
        }
        await self.eval_repo.create_dataset(
            {
                "dataset_id": dataset_id,
                "kb_id": kb_id,
                "name": name,
                "description": description,
                "item_count": 0,
                "has_gold_chunks": True,
                "has_gold_answers": True,
                "build_metadata": build_metadata,
                "created_by": created_by,
            }
        )
        task_id = f"task_{uuid.uuid4().hex[:8]}"
        try:
            self._spawn_task(
                dataset_id,
                self._generate_dataset_task(
                    task_id=task_id,
                    dataset_id=dataset_id,
                    kb_id=kb_id,
                    created_by=created_by,
                    name=name,
                    description=description,
                    count=count,
                    neighbors_count=neighbors_count,
                    concurrency_count=concurrency_count,
                    llm_model_spec=llm_model_spec,
                    generation_mode=generation_mode,
                    graph_expand_top_k=graph_expand_top_k,
                ),
            )
        except Exception:
            await self.eval_repo.update_dataset(
                dataset_id, {"build_metadata": {**build_metadata, "status": "failed", "message": "任务入队失败"}}
            )
            raise
        build_metadata["task_id"] = task_id
        await self.eval_repo.update_dataset(dataset_id, {"build_metadata": build_metadata})
        return {"dataset_id": dataset_id, "task_id": task_id, "message": "评估数据集生成任务已提交"}

    async def _update_dataset_build_metadata(
        self, dataset_id: str, metadata: dict[str, Any], **updates
    ) -> dict[str, Any]:
        metadata.update(updates)
        await self.eval_repo.update_dataset(dataset_id, {"build_metadata": metadata})
        return metadata

    async def _generate_dataset_task(
        self,
        *,
        task_id: str,
        dataset_id: str,
        kb_id: str,
        created_by: str,
        name: str,
        description: str,
        count: int,
        neighbors_count: int,
        concurrency_count: int,
        llm_model_spec: str,
        generation_mode: str,
        graph_expand_top_k: int,
    ) -> None:
        del created_by, name, description
        build_metadata = {
            "source": "generated",
            "status": "running",
            "progress": 0,
            "task_id": task_id,
            "params": {
                "count": count,
                "neighbors_count": neighbors_count,
                "concurrency_count": concurrency_count,
                "llm_model_spec": llm_model_spec,
                "generation_mode": generation_mode,
                "graph_expand_top_k": graph_expand_top_k,
            },
        }
        await self._update_dataset_build_metadata(dataset_id, build_metadata)

        async def report_progress(progress: float, message: str | None = None) -> None:
            await self._update_dataset_build_metadata(
                dataset_id,
                build_metadata,
                progress=max(0, min(round(progress), 100)),
                message=message or build_metadata.get("message", ""),
            )

        async def cancel_cb() -> None:
            task = asyncio.current_task()
            if task is not None and task.cancelling():
                raise asyncio.CancelledError

        try:
            kb_instance = await self._aget_kb(kb_id)
            if not kb_instance:
                await report_progress(100, "知识库不存在")
                raise ValueError("Knowledge Base not found")
            if kb_instance.kb_type != "local":
                await report_progress(100, "仅支持 local 类型知识库生成评估数据集")
                raise ValueError("Unsupported KB type for dataset generation")

            questions = []
            try:
                async for item in iter_generated_benchmark_items(
                    kb_instance=kb_instance,
                    kb_id=kb_id,
                    count=count,
                    neighbors_count=neighbors_count,
                    llm_model_spec=llm_model_spec,
                    concurrency_count=concurrency_count,
                    generation_mode=generation_mode,
                    graph_expand_top_k=graph_expand_top_k,
                    progress_cb=report_progress,
                    cancel_cb=cancel_cb,
                    llm_fn=self.select_model_fn,
                ):
                    questions.append(item)
            except ValueError as e:
                if str(e) == "No chunks found in knowledge base":
                    await report_progress(100, "知识库为空或未解析到chunks")
                raise

            if not questions:
                raise ValueError("未生成有效评估题目")

            await self.eval_repo.add_dataset_items(self._build_dataset_items(dataset_id, kb_id, questions))
            await self.eval_repo.update_dataset(dataset_id, {"item_count": len(questions)})
            await self._update_dataset_build_metadata(
                dataset_id,
                build_metadata,
                status="completed",
                progress=100,
                message="完成",
            )
        except asyncio.CancelledError:
            await self._update_dataset_build_metadata(
                dataset_id,
                build_metadata,
                status="failed",
                progress=100,
                error_message="任务已取消",
                message="任务已取消",
            )
            raise
        except Exception as e:
            await self._update_dataset_build_metadata(
                dataset_id,
                build_metadata,
                status="failed",
                progress=100,
                error_message=str(e),
                message=str(e),
            )
            raise

    # ------------------------------------------------------------------
    # 评估运行
    # ------------------------------------------------------------------

    async def run_evaluation(
        self,
        kb_id: str,
        dataset_id: str,
        name: str | None = None,
        model_config: dict[str, Any] | None = None,
        created_by: str = "system",
    ) -> str:
        try:
            run_id = f"run_{uuid.uuid4().hex[:8]}"
            run_name = self._normalize_run_name(name, run_id)
            dataset_row = await self.eval_repo.get_dataset(dataset_id)
            if dataset_row is None or dataset_row.kb_id != kb_id:
                raise ValueError("Dataset not found")
            if (dataset_row.build_metadata or {}).get("status", "completed") != "completed":
                raise ValueError("Dataset is not ready")

            # 配置加载顺序：schema 默认值（底座）→ 已保存 options（用户显式覆盖）→ model_config（API 调用方覆盖）
            # 与 aquery 内的合并语义保持一致，确保 schema 默认值变更对存量 KB 生效。
            kb_instance = await self._aget_kb(kb_id)
            retrieval_config = kb_instance._get_default_query_params(kb_id).get("options", {}) if kb_instance else {}
            try:
                kb_row = await self.kb_repo.get_by_kb_id(kb_id)
                if kb_row and kb_row.query_params:
                    saved_options = (
                        kb_row.query_params.get("options", {}) if isinstance(kb_row.query_params, dict) else {}
                    )
                    if saved_options:
                        retrieval_config.update(saved_options)
                logger.info(f"从知识库 {kb_id} 加载检索配置: {list(retrieval_config.keys())}")
            except Exception as e:  # noqa: BLE001 - 检索配置加载失败不阻断评估，降级默认配置
                logger.error(f"获取知识库检索配置失败: {e}")

            if model_config:
                retrieval_config.update(model_config)

            await self.eval_repo.create_run(
                {
                    "run_id": run_id,
                    "name": run_name,
                    "kb_id": kb_id,
                    "dataset_id": dataset_id,
                    "status": "running",
                    "retrieval_config": retrieval_config,
                    "metrics": {},
                    "overall_score": None,
                    "total_items": dataset_row.item_count or 0,
                    "completed_items": 0,
                    "started_at": utc_now_naive(),
                    "completed_at": None,
                    "created_by": created_by,
                }
            )

            # I-18 修复：入队失败必须将 run 标 failed——run 行先于任务创建，
            # 否则该 run 永远停留 running，结果页无限转圈
            try:
                self._spawn_task(
                    run_id,
                    self._run_evaluation_task(
                        run_id=run_id,
                        kb_id=kb_id,
                        dataset_id=dataset_id,
                        retrieval_config=retrieval_config,
                    ),
                )
            except Exception:
                await self.eval_repo.update_run(
                    run_id,
                    {"status": "failed", "metrics": {"error": "任务入队失败"}, "completed_at": utc_now_naive()},
                )
                raise
            return run_id
        except Exception as e:
            logger.error(f"启动评估失败: {e}")
            raise

    async def _run_evaluation_task(
        self,
        *,
        run_id: str,
        kb_id: str,
        dataset_id: str,
        retrieval_config: dict[str, Any],
    ) -> None:
        try:
            dataset_row = await self.eval_repo.get_dataset(dataset_id)
            if dataset_row is None or dataset_row.kb_id != kb_id:
                raise ValueError("Dataset not found")
            dataset_items = await self.eval_repo.list_all_dataset_items(dataset_id)
            if not dataset_items:
                raise ValueError("Dataset has no items")

            kb_instance = await self._aget_kb(kb_id)
            if not kb_instance:
                raise ValueError(f"Knowledge Base {kb_id} not found")

            judge_llm = None
            judge_init_error = None
            if dataset_row.has_gold_answers:
                judge_model_spec = retrieval_config.get("judge_llm") or retrieval_config.get("answer_llm")
                if judge_model_spec:
                    try:
                        logger.debug(f"Initializing Judge LLM: {judge_model_spec}")
                        judge_llm = self.select_model_fn(model_spec=judge_model_spec)
                    except Exception as e:  # noqa: BLE001 - 注入工厂可能抛任意异常，降级为不可用并记录
                        logger.error(f"Failed to load judge LLM: {e}")
                        judge_llm = None
                        # BUG-28 修复：记录 judge LLM 不可用状态，不静默通过
                        judge_init_error = str(e)

            all_retrieval_metrics = []
            all_answer_metrics = []
            total_items = len(dataset_items)

            async def update_run_db(status=None, completed=None, metrics=None, final_score=None):
                data = {}
                if status is not None:
                    data["status"] = status
                    if status in ["completed", "failed"]:
                        data["completed_at"] = utc_now_naive()
                if completed is not None:
                    data["completed_items"] = completed
                if metrics is not None:
                    data["metrics"] = metrics
                if final_score is not None:
                    data["overall_score"] = final_score
                if data:
                    await self.eval_repo.update_run(run_id, data)

            success_count = 0
            failure_count = 0

            # EVAL-1 修复：评估改为并发 worker 池（复用 benchmark_generation 的模式），
            # 替代逐题串行。worker 数受 retrieval/judge LLM 限速约束，默认 4。
            # 单题失败不中断（保留 BUG-22 行为），进度每 5 题更新一次。
            eval_queue: asyncio.Queue[tuple[int, Any] | None] = asyncio.Queue()
            for idx, item in enumerate(dataset_items):
                eval_queue.put_nowait((idx, item))
            for _ in range(EVAL_WORKER_COUNT):
                eval_queue.put_nowait(None)

            eval_state_lock = asyncio.Lock()
            completed_count = 0

            async def eval_worker() -> None:
                nonlocal success_count, failure_count, completed_count
                while True:
                    entry = await eval_queue.get()
                    if entry is None:
                        eval_queue.task_done()
                        return
                    index, item = entry
                    try:
                        question_data = {
                            "query": item.query_text,
                            "gold_chunk_ids": item.gold_chunk_ids or [],
                            "gold_answer": item.gold_answer,
                        }
                        question_result = await evaluate_question(
                            kb_instance=kb_instance,
                            kb_id=kb_id,
                            question_data=question_data,
                            retrieval_config=retrieval_config,
                            has_gold_chunks=dataset_row.has_gold_chunks,
                            has_gold_answers=dataset_row.has_gold_answers,
                            judge_llm=judge_llm,
                            answer_llm_fn=self.select_model_fn,
                        )

                        detail = question_result["detail"]
                        async with eval_state_lock:
                            if dataset_row.has_gold_chunks and question_data.get("gold_chunk_ids"):
                                all_retrieval_metrics.append(question_result["retrieval_scores"])
                            if dataset_row.has_gold_answers and question_data.get("gold_answer") and judge_llm:
                                all_answer_metrics.append(question_result["answer_scores"])

                        # EVAL-2: 写入 is_error 标记，支持 error_only 查询 SQL 下推
                        await self.eval_repo.upsert_run_item(
                            run_id=run_id,
                            item_index=index,
                            data={
                                "dataset_item_id": item.item_id,
                                "is_error": self._metrics_is_error(detail.get("metrics") or {}),
                                **detail,
                            },
                        )
                        async with eval_state_lock:
                            success_count += 1
                            completed_count += 1
                            current = completed_count
                    except Exception as item_err:  # noqa: BLE001 - 单题失败标记 is_error 继续评估，与 YUSU 语义一致
                        logger.warning(f"Eval item {index} failed: {item_err}")
                        async with eval_state_lock:
                            failure_count += 1
                            completed_count += 1
                            current = completed_count
                    finally:
                        eval_queue.task_done()

                    # 每 5 题或最后一题更新进度（持锁读取快照后释放锁再 await，避免锁内 IO）
                    if current % 5 == 0 or current == total_items:
                        self._run_states[run_id] = {
                            "progress": 10 + (current / total_items) * 80,
                            "message": f"评估 {current}/{total_items}",
                        }
                        await update_run_db(completed=current)

            workers = [asyncio.create_task(eval_worker()) for _ in range(EVAL_WORKER_COUNT)]
            await asyncio.gather(*workers)

            self._run_states[run_id] = {"progress": 95, "message": "计算最终指标"}
            overall_metrics, overall_score = aggregate_metrics(
                all_retrieval_metrics, all_answer_metrics, include_overall_score=True
            )
            # BUG-22 修复：根据成功/失败计数决定最终状态，仅当全部失败才标记 failed
            final_status = "completed" if success_count > 0 else "failed"
            # BUG-28 修复：将 judge LLM 状态写入 run 结果，避免静默通过
            final_metrics = dict(overall_metrics)
            if judge_init_error:
                final_metrics["judge_status"] = "unavailable"
            elif judge_llm:
                final_metrics["judge_status"] = "available"
            else:
                final_metrics["judge_status"] = "skipped"
            final_metrics["judge_error"] = judge_init_error
            await update_run_db(
                status=final_status,
                completed=total_items,
                metrics=final_metrics,
                final_score=overall_score,
            )
            self._run_states[run_id] = {"progress": 100, "message": "完成" if success_count > 0 else "全部失败"}
        except asyncio.CancelledError:
            raise
        except Exception as e:
            logger.error(f"Task failed: {e}")
            try:
                await self.eval_repo.update_run(
                    run_id,
                    {"status": "failed", "metrics": {"error": str(e)}, "completed_at": utc_now_naive()},
                )
            except Exception as exc:  # noqa: BLE001 - 兜底记录失败状态，不再向上抛
                logger.error(f"Error updating run record: {exc}")
            raise

    # ------------------------------------------------------------------
    # 运行结果查询
    # ------------------------------------------------------------------

    async def list_runs(self, kb_id: str) -> list[dict[str, Any]]:
        try:
            rows = await self.eval_repo.list_runs(kb_id)
            runs = []
            for row in rows:
                run = {
                    "run_id": row.run_id,
                    "name": self._run_name_from_row(row),
                    "dataset_id": row.dataset_id,
                    "status": row.status,
                    "started_at": format_utc_datetime(row.started_at),
                    "completed_at": format_utc_datetime(row.completed_at),
                    "total_items": row.total_items,
                    "completed_items": row.completed_items,
                    "overall_score": row.overall_score,
                    "retrieval_config": row.retrieval_config or {},
                    "metrics": row.metrics or {},
                }
                if row.status == "running":
                    if self._is_task_alive(row.run_id):
                        run.update(**self._run_states.get(row.run_id, {}))
                    elif (row.started_at is None) or ((utc_now_naive() - row.started_at).total_seconds() > 300):
                        # I-18 修复：running 但无活跃任务（进程重启丢失内存任务/
                        # 任务执行中硬崩溃）→ 收敛为 failed，避免僵尸 running 记录。
                        # 5 分钟年龄下限防止误杀刚创建、任务尚未入队的 run
                        run["status"] = "failed"
                        await self.eval_repo.update_run(
                            row.run_id,
                            {"status": "failed", "completed_at": utc_now_naive()},
                        )
                runs.append(run)
            return runs
        except Exception as e:
            logger.error(f"获取评估运行历史失败: {e}")
            raise

    async def get_run_results(
        self, kb_id: str, run_id: str, page: int = 1, page_size: int = 20, error_only: bool = False
    ) -> dict[str, Any]:
        if not re.match(r"^run_[a-f0-9]{8}$", run_id):
            raise ValueError("Invalid run_id format")
        row = await self.eval_repo.get_run(run_id)
        if row is None or row.kb_id != kb_id:
            # EVAL-4 修复：run 行可能尚未落库或已被清理，但进程内任务仍存活时
            # 返回实时运行状态，避免 running 中途查结果必 404。
            if self._is_task_alive(run_id):
                return {"run_id": run_id, "status": "running", **self._run_states.get(run_id, {})}
            raise ValueError(f"Run not found for {run_id}")

        start_idx = (page - 1) * page_size
        if error_only:
            # EVAL-2 修复：用 is_error 列 SQL 下推过滤+分页，替代全表扫描内存过滤
            total = await self.eval_repo.count_run_items_by_error(run_id, is_error=True)
            details = await self.eval_repo.list_run_items_by_error(
                run_id, is_error=True, offset=start_idx, limit=page_size
            )
            paged_items = [self._run_item_to_dict(item) for item in details]
        else:
            total = await self.eval_repo.count_run_items(run_id)
            details = await self.eval_repo.list_run_items(run_id, start_idx, page_size)
            paged_items = [self._run_item_to_dict(item) for item in details]
        return {
            "run_id": row.run_id,
            "name": self._run_name_from_row(row),
            "status": row.status,
            "started_at": format_utc_datetime(row.started_at),
            "completed_at": format_utc_datetime(row.completed_at),
            "total_items": row.total_items or 0,
            "completed_items": row.completed_items or 0,
            "overall_score": row.overall_score,
            "retrieval_config": row.retrieval_config or {},
            "metrics": row.metrics or {},
            "items": paged_items,
            "pagination": {
                "current_page": page,
                "page_size": page_size,
                "total": total,
                "total_pages": (total + page_size - 1) // page_size,
                "error_only": error_only,
            },
        }

    async def delete_run(self, kb_id: str, run_id: str) -> None:
        if not re.match(r"^run_[a-f0-9]{8}$", run_id):
            raise ValueError("Invalid run_id format")
        row = await self.eval_repo.get_run(run_id)
        if row is None or row.kb_id != kb_id:
            raise ValueError("Run not found")
        # 取消运行中的评估任务（进程内任务），避免孤儿任务
        if row.status == "running" and self._cancel_task(run_id):
            logger.info(f"已取消评估运行 {run_id} 的后台任务")
        self._run_states.pop(run_id, None)
        await self.eval_repo.delete_run(run_id)
        logger.info(f"成功删除评估运行: {run_id}")


__all__ = ["EVAL_WORKER_COUNT", "EvaluationService", "build_evaluation_run_name"]
