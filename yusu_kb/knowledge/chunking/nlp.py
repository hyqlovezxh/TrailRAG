"""Token counting, bullet detection and chunk-merge primitives.

Verbatim port of YUSU ``yuxi.knowledge.chunking.ragflow_like.nlp``.
"""
...
def count_tokens(text: str) -> int:
    """Approximate token count without extra dependencies."""
    if not text:
        return 0
    # English words + digits + CJK single characters
...
def hard_split_by_token_limit(text: str, chunk_token_num: int, hard_limit_token_num: int | None = None) -> list[str]:
    """Hard-split text by token limit; used as the last-resort guard after naive_merge.

    hard_limit_token_num only takes effect when explicitly passed, allowing a
    slightly oversized trailing chunk to stay intact; otherwise chunks never
    exceed chunk_token_num (historical behavior).
    """
...
def is_probable_heading_line(line: str) -> bool:
    ...
    # Table/HTML residue is usually not a heading.
    if re.search(r"</?(table|tr|td|th|caption|tbody|thead)[^>]*>", text, flags=re.IGNORECASE):
        return False

    # Overlong lines are usually body text or clauses, not chapter headings.
    if len(text) > 96:
        return False
    if count_tokens(text) > 72:
        return False

    # A heading rarely contains sentence punctuation early on.
...
def bullets_category(sections: list[str]) -> int:
    ...
    def bullet_weight(group_idx: int, line: str) -> float:
        # Weight markdown headings so body "一、/（一）" markers do not outrank real "#" levels.
    ...
    def add_chunk(text: str, pos: str) -> None:
        ...
        if chunks[-1] == "" or token_nums[-1] > threshold:
            if chunks:
                ...
                # Align back to a newline boundary to avoid cutting mid-record (table row key loss).
                boundary = prev.find("\n", start)
                if boundary != -1 and boundary < len(prev) - 1:
                    start = boundary + 1
