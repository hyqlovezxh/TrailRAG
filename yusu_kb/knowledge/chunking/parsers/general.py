"""General (naive) chunking: split by delimiter and length.

Ported from YUSU ``yuxi.knowledge.chunking.ragflow_like.parsers.general``.
"""
...
def _ensure_chunk_token_limit(chunks: list[str], chunk_token_num: int) -> list[str]:
    """Protect output chunks with a token upper bound: at the 1024-token default,
    a hard split is only applied above 1536 tokens."""
