"""Laws chunking: article-level tree organization and merge.

Ported from YUSU ``yuxi.knowledge.chunking.ragflow_like.parsers.laws``.
"""
...
def _normalize_law_line(line: str) -> str:
    # Common markdown decorations (#, -, **) on law lines interfere with level
    # detection; do a light normalization first.
    text = (line or "").strip()
...
def _ensure_chunk_token_limit(
    ...
    """Protect output chunks with a token upper bound:
    1) re-merge overlong chunks line-wise with naive_merge;
    2) only hard-split what still overflows, avoiding embedding 413s.
    """
...
def chunk_markdown(filename: str, markdown_content: str, parser_config: dict[str, Any] | None = None) -> list[str]:
    """Laws chunking main flow (simplified):
    - docx prefers the heading tree;
    - other formats normalize law text first, then tree-split by chapter/section/article (depth=3);
    - finally run the overlong protection uniformly.
    """
    parser_config = parser_config or {}

    delimiter = _unescape_delimiter(str(parser_config.get("delimiter", "\n") or "\n"))
    chunk_token_num = int(parser_config.get("chunk_token_num", 1024) or 1024)
    ...
    if bull == nlp.MARKDOWN_BULLET_GROUP_INDEX:
        # After normalization, falling back to the Chinese-law hierarchy group
        # avoids degrading into "one big block per chapter".
        bull = 0
