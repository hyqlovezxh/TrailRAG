"""HTML table conversion helpers.

Ported from YUSU ``yuxi.knowledge.chunking.ragflow_like.utils.table_utils``.
"""
...
def html_table_to_key_value(html: str) -> list[str]:
    """Convert an HTML table to key-value lines (handles rowspan/colspan).

    The first grid row is the header (keys); each following row becomes
    "key：value；key：value；" — built for chunking very long tables.
    """
    soup = BeautifulSoup(html, "html.parser")
    table = soup.find("table")
    if table is None:
